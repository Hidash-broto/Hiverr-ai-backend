...
export const getUsersFromContacts = async (req, res) => {
    try {
        const { contacts = [] } = req.body; // contacts: array of phone numbers from frontend
        if (!Array.isArray(contacts)) {
            return res.status(400).json({ status: false, error: "contacts must be an array" });
        }
        ...
        // Pre-normalize incoming contacts into a Set for O(1) lookups
        const normalizedContactsSet = new Set(
            contacts.map(normalizePhoneNumber).filter(Boolean)
        );

        // Fetch users with mobile and email
        const users = await User.find({}, 'mobile email');

        // Collect matching user emails
        const matchedEmails = [];
        for (const u of users) {
            const normMobile = normalizePhoneNumber(u.mobile);
            if (normMobile && normalizedContactsSet.has(normMobile) && u.email) {
                matchedEmails.push(u.email);
            }
        }

        // Return unique emails
        const uniqueEmails = [...new Set(matchedEmails)];
        res.json({ status: true, users: uniqueEmails });
    } catch (err) {
